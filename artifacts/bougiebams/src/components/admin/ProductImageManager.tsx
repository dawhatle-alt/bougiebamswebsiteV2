...
export default function ProductImageManager({ token, onAuthError }: Props) {
  ...
  if (loading) {
    return (
      <div className="py-20 text-center text-[#5A6178]">Loading products…</div>
    );
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {products.map((product) => {
          ...
          return (
            <div
              key={product.sku}
              className="rounded-lg border border-[#E2DBCD] bg-white overflow-hidden"
            >
              ...
              <div className="p-3">
                <p className="font-medium text-[#1E2A5A] text-sm leading-tight truncate">
                  {product.name}
                </p>
                <p className="text-xs text-[#9A8F7E] mt-0.5 truncate">
                  SKU: {product.sku || "—"}
                  {hasOverride && (
                    <span className="ml-2 text-[#D4AF37]">● Custom</span>
                  )}
                </p>
                <Button
                  size="sm"
                ...
                >
                  {isUploading ? (
                    ...
                    <>
                      ...
                      {hasOverride ? "Replace image" : "Upload image"}
                    </>
                  )}
                </Button>
              </div>
