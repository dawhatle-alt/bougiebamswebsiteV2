...
router.post("/registrations/checkout", async (req: Request, res: Response): Promise<void> => {
...
router.post(
  ...
  async (req: Request & { rawBody?: Buffer }, res: Response): Promise<void> => {
    ...
    try {
      const Stripe = (await import("stripe")).default;
      const stripe = new Stripe(stripeKey);
      stripeEvent = stripe.webhooks.constructEvent(req.rawBody, sig as string, webhookSecret);
    } catch (err) {
